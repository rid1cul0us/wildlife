...
def draw_point2labelcnt(point2labelcnt):
    ...
    plt.savefig(
        f"pic_zh/number_of_points_with_the_label_count_of_each_point{EXT}", dpi=300
    )
    plt.clf()
...
def draw_label2ponitcnt(label2pointcnt):
    ...
    plt.savefig(
        f"pic_zh/number_of_labels_with_the_number_of_presented_points_of_each_species{EXT}",
        dpi=300,
    )
    plt.clf()
...
def qualitative_correlation_analysis(
    ...
    ax1.set_ylabel("累计物种数", fontsize=12)
    ax1.set_ylim(20, 200)
    ...
    # species_bins = [0, 10, 20, 30, len(labels)-1]
    for i in range(len(hist)):
    ...
    ax2.set_ylabel("累计点位数", fontsize=12)
    ax2.set_ylim(50, 350)

    xtick_labels = [
        "",
    ] * length
    xtick_labels[label_point_count_pos] = "各物种出现的点位数量"
    xtick_labels[point_label_count_pos] = "各点位目击的物种数量"
    plt.xticks(range(length), xtick_labels)
    plt.setp(ax.get_xticklabels(), fontsize=12)
    # ax2.set_xticklabels(xtick_labels, fontsize=14)
    # plt.xlim(0, 4)
    plt.title("物种与点位间的相关性分析", fontsize=14)
    plt.tight_layout()
    # plt.subplots_adjust(hspace=0.5)

    plt.savefig(f"pic_zh/物种与点位间的相关性分析{EXT}", dpi=300)
    # plt.show()
    plt.clf()
...
def calculate_entropy(label2point_dist, point2label_dist):
    ...
    os.makedirs("pic_zh/point_distribution/", exist_ok=True)
    for y, point_distribution in enumerate(label2point_dist):
        ...
        # plt.savefig(f'./pic_zh/point_distribution/{y}_{category_id2name[y2category_id[y]]}.png')
        # plt.clf()

    # point_entropy_of_labels
    labels_entropy_of_points = []
    os.makedirs("pic_zh/label_distribution/", exist_ok=True)
    for point, label_distribution in enumerate(point2label_dist):
        ...
        # plt.savefig(f'./pic_zh/label_distribution/{point}.png')
        # plt.clf()
    ...
    plt.savefig(f"pic_zh/point_entropy_of_species_distribution{EXT}", dpi=300)
    plt.clf()
    ...
    plt.savefig(f"pic_zh/species_entropy_of_points_distribution{EXT}", dpi=300)
    plt.clf()
...
def softmax_weights_visualization(weights_path, label2point_dist, points, labels):
    ...
    plt.tight_layout()
    plt.savefig(f"pic_zh/weights_visualization_{weights_path}.png", dpi=300)
...
if __name__ == "__main__":
    ...
    # draw
    os.makedirs("pic_zh", exist_ok=True)
